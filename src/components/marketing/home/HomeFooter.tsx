...
const FALLBACK_COLUMNS = [
  ...
  },
  {
    heading: "Resources",
    links: [
      { label: "For Providers", href: "#about" },
...
async function getFooterSettings() {
  try {
    ...
    return settings?.columns?.length ? settings.columns : FALLBACK_COLUMNS;
  } catch {
...
export default async function HomeFooter() {
  const columns = await getFooterSettings();

  return (
    <footer className="bg-[#27317B] py-12 text-white">
      <div className="mx-auto grid max-w-6xl gap-10 px-6 sm:grid-cols-2 lg:grid-cols-4 lg:px-10">
        ...
        {columns.map((column) => (
          <div key={column.heading}>
            <p className="mb-3 text-sm font-bold text-white">{column.heading}</p>
