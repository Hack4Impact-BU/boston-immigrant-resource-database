...
import { updateFeedbackFieldAction } from "@/features/admin/feedback/manage-feedback";
import type { AdminFeedbackRecord, FieldOptionColors } from "@/lib/airtable";
...
// Multi-line text columns (Feedback Text, Admin Notes) wrap across several lines
...
const FILTERABLE_FIELDS = ["Feedback Type", "Status"];
const SEARCHABLE_FIELDS = ["Admin Notes", "Submitted By", "Feedback Text"];

function paddingForColumn(column: FeedbackColumn): number {
...
function getDisplayValue(entry: AdminFeedbackRecord, column: FeedbackColumn): string {
  if (column.widget === "date") return formatDateValue(entry.fields[column.field]);
  return toDisplayString(entry.fields[column.field]);
}
...
function computeEstimatedWidths(feedback: AdminFeedbackRecord[], columns: FeedbackColumn[]): Record<string, number> {
  ...
  for (const column of columns) {
    ...
    const longest = Math.max(column.label.length, ...feedback.map((r) => getDisplayValue(r, column).length));
    widths[column.field] = Math.max(MIN_COLUMN_WIDTH, longest * FALLBACK_CHAR_WIDTH + paddingForColumn(column));
  }
  return widths;
}

function computeMeasuredWidths(feedback: AdminFeedbackRecord[], columns: FeedbackColumn[]): Record<string, number> {
  ...
  for (const column of columns) {
    ...
    const longestCellWidth = Math.max(0, ...feedback.map((r) => measureTextWidth(getDisplayValue(r, column), cellFont)));
...
function EditableTextCell({ recordId, fieldName, initialValue, multiline }: { recordId: string; fieldName: string; initialValue: string; multiline?: boolean }) {
  ...
  async function handleBlur() {
    ...
    setSaveState("saving");
    try {
      await updateFeedbackFieldAction({ recordId, fieldName, value: trimmed });
      setValue(trimmed);
...
function EditableSelectCell({
  ...
  async function handleChange(newValue: string) {
    ...
    setSaveState("saving");
    try {
      await updateFeedbackFieldAction({ recordId, fieldName, value: newValue });
      setSaveState("saved");
    } catch (error) {
      console.error(error);
...
function EditableCheckboxCell({ recordId, fieldName, initialValue }: { recordId: string; fieldName: string; initialValue: boolean }) {
  ...
  async function handleChange(newValue: boolean) {
    ...
    setSaveState("saving");
    try {
      await updateFeedbackFieldAction({ recordId, fieldName, value: newValue ? "true" : "" });
      setSaveState("saved");
    } catch (error) {
      console.error(error);
...
export default function FeedbackTable({
  ...
  useEffect(() => {
    setColumnWidths(computeMeasuredWidths(feedback, columns));
  ...
  const filteredFeedback = useMemo(() => {
    ...
    if (activeEntries.length === 0) return feedback;

    return feedback.filter((entry) =>
      activeEntries.every(([field, values]) => {
        const cellValue = toDisplayString(entry.fields[field]);
        return values.includes(cellValue);
      }),
    );
  }, [feedback, activeFilters]);

  const searchedFeedback = useMemo(() => {
    const query = searchText.trim().toLowerCase();
    if (!query) return filteredFeedback;
    const searchableColumns = columns.filter((column) => SEARCHABLE_FIELDS.includes(column.field));
    return filteredFeedback.filter((entry) =>
      searchableColumns.some((column) => getDisplayValue(entry, column).toLowerCase().includes(query)),
    );
  }, [filteredFeedback, columns, searchText]);

  const sortedFeedback = useMemo(() => {
    if (!sort) return searchedFeedback;
    const column = columns.find((c) => c.field === sort.field);
    if (!column) return searchedFeedback;

    const withValues = searchedFeedback.map((entry) => ({
      entry,
      value: column.widget === "date"
        ? new Date(toDisplayString(entry.fields[sort.field])).getTime() || 0
        : getDisplayValue(entry, column).toLowerCase(),
    }));
    ...
    return withValues.map((entry) => entry.entry);
  }, [searchedFeedback, sort, columns]);
    ...
    <div className="flex h-full min-h-0 flex-col gap-3">
      <div className="relative shrink-0">
        <input
          ...
          placeholder="Search by Admin Notes, Submitted By, or Feedback Text…"
        ...
        <table className="border-collapse text-left" style={{ tableLayout: "fixed", width: "max-content" }}>
          ...
          <tbody>
            {sortedFeedback.map((entry) => (
              <tr key={entry.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/60">
                {columns.map((column) => {
                  const rawValue = entry.fields[column.field];

                  if (!column.editable || column.widget === "date") {
                    return (
                      <td key={column.field} className="h-px overflow-hidden px-1 py-1">
                        <ReadOnlyCell
                          value={getDisplayValue(entry, column)}
                  ...
                  if (column.widget === "checkbox") {
                    return (
                      <td key={column.field} className="h-px overflow-hidden px-1 py-1">
                        <EditableCheckboxCell recordId={entry.id} fieldName={column.field} initialValue={Boolean(rawValue)} />
                      </td>
                    );
                  }
